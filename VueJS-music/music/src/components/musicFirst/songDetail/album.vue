...
export default {
  ...
  mounted() {
    ...
    if (localStorage.getItem("qqSingerList")) {
      this.numId = JSON.parse(localStorage.getItem("qqSingerList")).songId;
      listId = JSON.parse(localStorage.getItem("qqSingerList")).songlistId;
      if (localStorage.getItem("qqsingerCurrent")) {
        this.playTime = localStorage.getItem("qqsingerCurrent");
      }
    } else {
      this.numId = this.$route.params.nowId || "12429072";
      listId = this.$route.params.albumId || "1346104327";
  ...
  methods: {
        ...
        for (var i = 0; i < this.channelArr[0].length; i++) {
          if (this.channelArr[0][i].id === this.numId) {
            if (i > 0) {
              ...
              this.numId = this.channelArr[0][this.channelArr.length - 1].id;
              break;
            }
          }
        }
    ...
    getChannel(listId) {
      ...
      this.$axios
        .get(this.tosinger2, { params: { id: listId } })
        .then(res => {
          //console.log(res);
          this.channelArr.push(res.data.hotSongs);
          ...
          localStorage.setItem("qqSingerList", songlistdata);
        })
        .catch(error => {
          console.log(error);
        });
    ...
    getMusic(numId) {
      ...
      if(localStorage.getItem("qqSingerList")){
        let songlistdata=JSON.parse(localStorage.getItem("qqSingerList"));
        songlistdata.songId=numId;
        localStorage.setItem("qqSingerList",JSON.stringify(songlistdata));
      }
      this.$axios
